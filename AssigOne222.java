...
public class AssigOne222
{
    public static void main(String[] args)
    {
        ...
        for (int i = 1; i <= 180; i++)
        {
            ...
            if (i <= 90 && i > 0)
            {
                double laserMoveTimeInSeconds = (90 - i) * LASERTRAVELTIME; // time for laser to move to degree i when the laser is moving left
            ...
            {
                double laserMoveTimeInSeconds = (i * LASERTRAVELTIME); /// time for laser to move to degree i when the laser is moving right
                laserTimeInSeconds = Math.max(laserMoveTimeInSeconds, LASERCHARGETIME); // make sure the laser has had time to charge
            ...
            if (laserTimeInSeconds <= missileTimeInSeconds && i < 180) 
            {
                ...
                degreesLastFired = degrees; // sets current degree as the degree the laser was last fired at
        ...
        {            
            int degreesSinceLastFired = i - degreesLastFired; // sets the degrees since the laser was last fired
            ...
            if (missileTimeInSeconds >= Math.max(missileTimeInSeconds, timeLastFired + LASERCHARGETIME))
            {
               Time firingTime = new Time(12, 30, 0); // reset time to initial launch time
                ...
                degreesLastFired = i; // set degrees laser was last fired at to current position
