...
public class AssigOne222
{
    public static void main(String[] args)
    {
        ...
        double timeLastFired = LAUNCHTIME;
        ...
        for (int i = 1; i <= 180; i++)
        {
            ...
            // time for laser to move to missile position ( missile position = i)
            if (i <= 90 && i > 0)
            {
                double laserMoveTimeInSeconds = (90 - i) * LASERTRAVELTIME; // time it would take for laser to move from start position to i
            ...
            {
                double laserMoveTimeInSeconds = (i * LASERTRAVELTIME); // time it would take for laser to move from start position to i
                laserTimeInSeconds = Math.max(laserMoveTimeInSeconds, LASERCHARGETIME); // make sure the laser has had time to charge
            ...
            if (laserTimeInSeconds <= missileTimeInSeconds && i < 180) 
            {
                ...
                firstFireTime.addSeconds((int) missileTimeInSeconds);
                ...
                FiringSolution fireLaser = new FiringSolution(firstFireTime, degrees, x, y);
        ...
        {            
            ...
            {
                // fire
                Time firingTime = new Time(12, 30, 0);
                firingTime.addSeconds((int) (missileTimeInSeconds));
               
                double degreeInRad = Math.toRadians(i);
                double x = 4000 * Math.cos(degreeInRad); // calculate x co-ordinate
                double y = 20 * Math.sin(degreeInRad); // calculate y co-ordinate

                FiringSolution fireLaser = new FiringSolution(firingTime, i, x, y);
                System.out.println(fireLaser.toString());  
